...
local function New(ClassName: string, Properties: { [string]: any }): any
    ...
    if Templates[ClassName] then
        pcall(FillInstance, Templates[ClassName], CreatedInstance)
    end
    pcall(FillInstance, Properties, CreatedInstance)
...
--// Main Instances \\-
local function SafeParentUI(GuiInstance: Instance, Parent: Instance | () -> Instance)
    ...
    local function ResolveParent()
        ...
        if typeof(Parent) == "function" then
            local Success, Result = pcall(Parent)
    ...
    local function TrySetParent(DestinationParent)
        if not DestinationParent then
            return false
        end
        local Success = pcall(function()
            GuiInstance.Parent = DestinationParent
        end)
        if not Success then
            return false
        end
        ...
        pcall(function()
            HasParent = GuiInstance.Parent ~= nil
        end)
        return HasParent
    end

    for _ = 1, 6 do
        if TrySetParent(ResolveParent()) then
            return true
        end
        task.wait()
    end
    ...
    local PlayerGui
    pcall(function()
        PlayerGui = Library.LocalPlayer:WaitForChild("PlayerGui", 10)
    end)

    for _ = 1, 6 do
        if TrySetParent(PlayerGui) then
            return true
        end
        task.wait()
    end
...
local function ParentUI(UI: Instance, SkipHiddenUI: boolean?)
    ...
    if SkipHiddenUI then
        return SafeParentUI(UI, CoreGui)
    end

    pcall(protectgui, UI)
    return SafeParentUI(UI, gethui)
end
...
function Library:SetNotifySide(Side: string)
    ...
    local function ApplyNotifySide()
        if not NotificationArea or not NotificationList then
            return false
        end
    ...
    local Success = pcall(ApplyNotifySide)
    if not Success then
        task.defer(function()
            pcall(ApplyNotifySide)
        end)
    end
end
